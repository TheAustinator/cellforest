...
def plot_shell_command(command_string, working_dir, process_name):
    os.makedirs(working_dir, exist_ok=True)
    out_path = os.path.join(working_dir, "plots.out")
    err_path = os.path.join(working_dir, "plots.err")
    logger = logging.getLogger("plots:shell command")
    logger.info(f"Running command: {command_string}")
    logger.info(f"STDOUT -> {out_path}")
    logger.info(f"STDERR -> {err_path}")
    check_call(
