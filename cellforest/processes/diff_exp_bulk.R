...
# Make sure the rows in metadata[group_labels] are pulled out in the same order as the cell labels
# in the seurat_object. Remember, there's no guarantee they're in the same order and metadata[group_labels]
...
markers <- FindMarkers(seurat_object, ident.1 = ident1, ident.2 = ident2, test.use = test, logfc.threshold = logfc_thresh) # group.by=group_labels, subset.ident=value,
